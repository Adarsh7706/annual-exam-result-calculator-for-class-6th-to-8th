...
function calculateGrades() {
 ...
 // Calculate total marks
  const totalMarks = subject1 + subject2 + subject3 + subject4 + subject5 + subject6 + subject7 + subject8;
  const percentage = (totalMarks*100/600).toFixed(2);

 // Determine the grade
 let grade;
      if (totalMarks >= 575) {
        grade = 'A+';
      } else if (totalMarks >= 550) {
        grade = 'A';
      } else if (totalMarks >= 525) {
        grade = 'B';
